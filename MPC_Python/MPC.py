...
class ModelPredictiveControl(object):
    ...
    # zd - desiredControlTrajectory

    def __init__(self, A, B, C, f, v, W3, W4, x0, zd_total):
        self.A = A
        self.B = B
        self.C = C
        self.f = f
            ...
            if (i < v):
                ...
                for j in range(v):
                    if j == 0:
                        ...
                        M[i * r:(i + 1) * r, (i - j) * m:(i - j + 1) * m] = np.matmul(C, np.matmul(sumLast,B))
                    else:
                        powA = np.matmul(powA,A)
                        M[i * r:(i + 1) * r, (v - 1 - j) * m:(v - j) * m] = np.matmul(C, np.matmul(powA,B))


        temp1 = np.matmul(M.T, np.matmul(self.W4, M))
        temp2 = np.linalg.inv(temp1 + self.W3)
        gainMatrix = np.matmul(temp2, np.matmul(M.T, self.W4))
    ...
    def computeControlInputs(self):
        zd = self.zd_total[self.currentTimeStep:self.currentTimeStep+self.f, :]
        vectorS = zd - np.matmul(self.O, self.states[self.currentTimeStep])
